...
| Future runtime theming | ✅ `applyWhiteLabel()` overrides inline styles on top | ✅ Also works |
...
1. **Design system agnostic** — CSS custom properties are a web standard. When switching to a different design system, only the variable names in `bahmni-tokens.scss` change. `_palette.scss` hex values and the `applyWhiteLabel()` utility both survive the switch untouched.
...
//   - Compatible with runtime theming: applyWhiteLabel() can override further via inline styles
//
...
- `applyWhiteLabel()` utility — **untouched**. It is a pure DOM function with no design system dependency.
...
applyWhiteLabel() (DOM)      returns plain object           .then(applyWhiteLabel)
No service calls ever         No DOM access                  .catch(() => {})
```

`applyWhiteLabel()` sets inline styles on `document.documentElement` — inline styles have higher specificity than any stylesheet rule and cleanly override the static defaults from `bahmni-tokens.scss`:

```ts
// packages/bahmni-design-system/src/utils/applyWhiteLabel.ts
...
export async function fetchThemeConfig(): Promise<Partial<WhiteLabelConfig>> {
...
| Future 7 | **Create** | `packages/bahmni-design-system/src/utils/applyWhiteLabel.ts` |
