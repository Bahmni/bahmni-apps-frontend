...
describe('PatientSearch Component', () => {
  ...
  const defaultPhoneSearchReturn = {
    searchResults: [],
    loading: false,
    error: null,
  };
  ...
  beforeEach(() => {
    ...
    mockedUsePhoneNumberSearch.mockReturnValue(defaultPhoneSearchReturn);
  });
  ...
  it('should call onSearchResults when phone search button is clicked', () => {
  ...
  it('should call onError when phone search returns an error', () => {
    const phoneErrorMessage = 'Phone search failed';
    mockedUsePhoneNumberSearch.mockReturnValue({
      searchResults: [],
      loading: false,
    ...
    expect(mockOnError).toHaveBeenCalledWith(phoneErrorMessage);
  });
});
