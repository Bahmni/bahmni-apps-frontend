import { logAuditEvent, logDashboardView, logEncounterEdit } from '../auditLogService';
...
const mockIsAuditLogEnabled = isAuditLogEnabled as jest.MockedFunction<typeof isAuditLogEnabled>;
...
describe('auditLogService', () => {
  ...
  describe('logAuditEvent', () => {
    it('should return logged false without logging when audit logging is disabled', async () => {
      ...
      const result = await logAuditEvent('patient-123', 'VIEWED_CLINICAL_DASHBOARD');
    ...
    it('should log audit event when audit logging is enabled', async () => {
      ...
      expect(mockClient.post).toHaveBeenCalledWith(AUDIT_LOG_URL, {
    ...
    it('should include message parameters in audit log', async () => {
      ...
      const messageParams = { encounterUuid: 'encounter-123', encounterType: 'Consultation' };
      const result = await logAuditEvent('patient-123', 'EDIT_ENCOUNTER', messageParams);
    ...
    it('should handle unknown event types', async () => {
      ...
      const result = await logAuditEvent('patient-123', 'UNKNOWN_EVENT' as AuditEventType);

      expect(result).toEqual({ logged: false, error: 'Unknown audit event type: UNKNOWN_EVENT' });
      expect(mockClient.post).not.toHaveBeenCalled();
    });
    ...
    it('should use custom module when provided', async () => {
      ...
      const result = await logAuditEvent('patient-123', 'VIEWED_CLINICAL_DASHBOARD', undefined, 'CUSTOM_MODULE');
    ...
    it('should handle undefined patient UUID', async () => {
      ...
      const result = await logAuditEvent(undefined, 'VIEWED_CLINICAL_DASHBOARD');
  ...
  describe('logEncounterEdit', () => {
    it('should call logAuditEvent with correct parameters and message params', async () => {
      ...
      const result = await logEncounterEdit('patient-123', 'encounter-456', 'Consultation');
