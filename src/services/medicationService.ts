...
export async function searchMedications(searchTerm: string, count: number = 20): Promise<Bundle<Medication>> {
  return await get<Bundle<Medication>>(MEDICATIONS_SEARCH_URL(searchTerm, count));
}

export function getMedicationDisplay(medication: Medication): string {
  const medicationExtensions = medication.extension?.find((ext) => ext.url === FHIR_MEDICATION_EXTENSION_URL);
